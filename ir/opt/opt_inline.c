	...
	for (i = 0; i < n_params; ++i) {
		...
		if (is_Const(param)) {
			...
			if (is_SymConst(param))
				 ...
				 */
				unsigned v = get_method_local_address_weight(called, i);
				weight += v;
			}
		}
	}
...
static int calc_call_dynamic_benefice(call_env_t *cenv)
{
	...
	if (called_env->n_blocks == 1)
		...
		weight += block_function_weight * called_env->n_blocks;
	...
	weight += called_env->n_nodes * large_function_weight;
